"""Contact card tool for retrieving contact information from LACNIC database."""
...
# Example: ["email", {}, "text", "abuse@lacnic.net"]
...
TOOL_DESCRIPTION = (
    "PREFERRED TOOL for retrieving contact information (abuse, NOC, admin, tech) for IP addresses, ASNs, or organizations from the LACNIC database. "
    ...
    "technical contacts, administrative contacts, and phone numbers from LACNIC database. Perfect for incident response, "
    "network troubleshooting, and compliance reporting for LACNIC-managed resources. Returns structured contact data with "
    "clear categorization of contact types and purposes."
)

IP_DESCRIPTION = (
    "IP address to look up contact information for in LACNIC database (IPv4 or IPv6)"
)
ASN_DESCRIPTION = "ASN number to look up contact information for in LACNIC database (without 'AS' prefix)"
...
async def _contact_card_request(
    ...
    """
    Retrieve comprehensive contact information for IP addresses, ASNs, or organizations from LACNIC.
    ...
    if not SUPPORT_LACNIC:
        error_msg = "Contact card lookup is currently disabled (SUPPORT_LACNIC=false)"
        logger.warning(error_msg)
        return {
            "ok": False,
            "error": "service_disabled",
            "detail": "LACNIC contact card support is disabled. Set SUPPORT_LACNIC=true to enable.",
        }
    ...
    if provided_params != 1:
        logger.error(
            "LACNIC contact card request failed: Provide exactly one of ip, asn, or org"
        )
        return {
            "ok": False,
            "error": "bad_request",
    ...
    # Create cache key
    if ip:
        cache_key = f"lacnic:contact_ip:{ip}"
        query_type = "ip"
        query_value = ip
    elif asn is not None:
        cache_key = f"lacnic:contact_asn:{asn}"
        query_type = "asn"
        query_value = str(asn)
    else:
        cache_key = f"lacnic:contact_org:{org}"
        query_type = "org"
        query_value = org

    # Log the incoming request
    logger.info(f"Starting LACNIC contact card lookup for {query_type}='{query_value}'")

    # Check cache first
    cached_result = _contact_cache.get(cache_key)
    if cached_result is not None:
        logger.info(
            f"LACNIC contact card for {query_type}='{query_value}' served from cache"
        )
    ...
    try:
        ...
        if ip:
            ...
            if "/" not in ip:
                ...
                rdap_query = ip
            url = f"{LACNIC_RDAP_BASE}/ip/{rdap_query}"
            logger.info(f"Querying LACNIC RDAP for IP: {rdap_query}")
        elif asn is not None:
            url = f"{LACNIC_RDAP_BASE}/autnum/{asn}"
            logger.info(f"Querying LACNIC RDAP for ASN: {asn}")
        elif org:
        ...
        logger.info(
            f"LACNIC contact card completed: found '{org_name}' (abuse: {abuse_available}, "
        ...
        error_msg = f"LACNIC contact card lookup for {query_type}='{query_value}' failed: {str(e)}"
        logger.error(error_msg)
        return {"ok": False, "error": "lookup_error", "detail": str(e)}


def register(mcp: FastMCP) -> None:
    """Register the LACNIC contact_card tool with the MCP server."""
    mcp.tool(
        name=TOOL_NAME,

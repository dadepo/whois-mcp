"""Contact card tool for retrieving contact information from AfriNIC database."""
...
# Example: ["email", {}, "text", "abuse@afrinic.net"]
...
TOOL_DESCRIPTION = (
    "PREFERRED TOOL for retrieving contact information (abuse, NOC, admin, tech) for IP addresses, ASNs, or organizations from the AfriNIC database. "
    ...
    "technical contacts, administrative contacts, and phone numbers from AfriNIC database. Perfect for incident response, "
    "network troubleshooting, and compliance reporting for AfriNIC-managed resources. Returns structured contact data with "
    "clear categorization of contact types and purposes."
)

IP_DESCRIPTION = (
    "IP address to look up contact information for in AfriNIC database (IPv4 or IPv6)"
)
ASN_DESCRIPTION = "ASN number to look up contact information for in AfriNIC database (without 'AS' prefix)"
...
async def _contact_card_request(
    ...
    """
    Retrieve comprehensive contact information for IP addresses, ASNs, or organizations from AfriNIC.
    ...
    if not SUPPORT_AFRINIC:
        error_msg = "Contact card lookup is currently disabled (SUPPORT_AFRINIC=false)"
        logger.warning(error_msg)
        return {
            "ok": False,
            "error": "service_disabled",
            "detail": "AfriNIC contact card support is disabled. Set SUPPORT_AFRINIC=true to enable.",
        }
    ...
    if provided_params != 1:
        logger.error(
            "AfriNIC contact card request failed: Provide exactly one of ip, asn, or org"
        )
        return {
            "ok": False,
            "error": "bad_request",
    ...
    # Create cache key
    if ip:
        cache_key = f"afrinic:contact_ip:{ip}"
        query_type = "ip"
        query_value = ip
    elif asn is not None:
        cache_key = f"afrinic:contact_asn:{asn}"
        query_type = "asn"
        query_value = str(asn)
    else:
        cache_key = f"afrinic:contact_org:{org}"
        query_type = "org"
        query_value = org
    ...
    if cached_result is not None:
        logger.info(
            f"AfriNIC contact card for {query_type}='{query_value}' served from cache"
        )
    ...
    try:
        ...
        if ip:
            ...
            if "/" not in ip:
                ...
                rdap_query = ip
            url = f"{AFRINIC_RDAP_BASE}/ip/{rdap_query}"
            logger.info(f"Querying AfriNIC RDAP for IP: {rdap_query}")
        elif asn is not None:
            url = f"{AFRINIC_RDAP_BASE}/autnum/{asn}"
            logger.info(f"Querying AfriNIC RDAP for ASN: {asn}")
        elif org:
        ...
        logger.info(
            f"AfriNIC contact card completed: found '{org_name}' (abuse: {abuse_available}, "
        ...
        error_msg = f"AfriNIC contact card lookup for {query_type}='{query_value}' failed: {str(e)}"
        logger.error(error_msg)
        return {"ok": False, "error": "lookup_error", "detail": str(e)}


def register(mcp: FastMCP) -> None:
    """Register the AfriNIC contact_card tool with the MCP server."""
    mcp.tool(
        name=TOOL_NAME,

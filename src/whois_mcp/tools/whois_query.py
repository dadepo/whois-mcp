...
async def _whois_request(
    query: Annotated[str, Field(description="The domain name, IP address, or other identifier to query via WHOIS")],
    flags: Annotated[Optional[List[str]], Field(default=None, description="Optional WHOIS flags to modify the query (e.g., ['-B'] for brief output, ['-r'] for raw output)")] = None
    ...
    try:
        ...
        return {"ok": False, "error": "timeout_error", "detail": "Connection or read timeout"}
